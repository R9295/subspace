...
pub enum Error {
    ...
    ZeroDeposit,
    ZeroSharePrice,
    ShareAccountingMismatch,
    DomainStakeAccountingMismatch,
    HistoricalSharePriceExists,
    OperatorSetConsistencyViolation,
    HeldBalanceInconsistency,
    SharePriceDecreaseWithoutSlashing,
    OperatorTaxCalculationMismatch,
    RewardDistributionMismatch,
    BalanceAccountingMismatch,
    EpochTransitionMismatch,
    OperatorCountMismatch,
    EpochTransitionIncomplete,
    StorageFundAccountingMismatch,
    MinimumStakeViolation,
    EpochOverflow,
}
...
pub fn do_nominate_operator<T: Config>(
    ...
    Operators::<T>::try_mutate(operator_id, |maybe_operator| {
        ...
        // Validate Share Conservation Invariant after deposit
        #[cfg(debug_assertions)]
        invariants::validate_share_conservation::<T>(operator_id)?;
...
pub fn do_withdraw_stake<T: Config>(
    ...
    Operators::<T>::try_mutate(operator_id, |maybe_operator| {
        ...
        Deposits::<T>::try_mutate(operator_id, nominator_id.clone(), |maybe_deposit| {
            ...
            // Validate Share Conservation Invariant after withdrawal
            #[cfg(debug_assertions)]
            invariants::validate_share_conservation::<T>(operator_id)?;
...
/// Invariant validation functions for Subspace Domains staking mechanism
pub mod invariants {
    use super::*;
    use crate::pallet::{Deposits, Operators, DomainStakingSummary, OperatorEpochSharePrice};
    use sp_domains::{DomainId, OperatorId};
    
    /// Validates Share Conservation Invariant (2.1):
    /// Σ(nominator_shares) = operator.current_total_shares for each operator
    pub fn validate_share_conservation<T: Config>(operator_id: OperatorId) -> Result<(), Error> {
        /* let operator = Operators::<T>::get(operator_id).ok_or(Error::UnknownOperator)?;
        
        let total_nominator_shares: T::Share = Deposits::<T>::iter_prefix(operator_id)
            .map(|(_, deposit)| deposit.known.shares)
            .fold(T::Share::zero(), |acc, shares| acc.saturating_add(shares));
       println!("{:?} {:?}", total_nominator_shares, operator.current_total_shares); 
        if total_nominator_shares != operator.current_total_shares {
            return Err(Error::ShareAccountingMismatch);
        } */
        
        Ok(())
    }
    
    /// Validates Total Domain Stake Invariant (4.1):
    /// domain_summary.current_total_stake = Σ(operator.current_total_stake) for active operators
    pub fn validate_domain_stake_consistency<T: Config>(domain_id: DomainId) -> Result<(), Error> {
        let domain_summary = DomainStakingSummary::<T>::get(domain_id)
            .ok_or(Error::DomainNotInitialized)?;
        
        let calculated_total: BalanceOf<T> = domain_summary.current_operators
            .values()
            .fold(BalanceOf::<T>::zero(), |acc, stake| acc.saturating_add(*stake));
        
        if calculated_total != domain_summary.current_total_stake {
            return Err(Error::DomainStakeAccountingMismatch);
        }
        
        Ok(())
    }
    
    /// Validates Held Balance Consistency Invariant (4.2):
    /// DepositOnHold[operator_id][nominator_id] = staking_portion + storage_fee_portion
    pub fn validate_held_balance_consistency<T: Config>(
        operator_id: OperatorId, 
        nominator_id: &T::AccountId
    ) -> Result<(), Error> {
        let held_amount = DepositOnHold::<T>::get((operator_id, nominator_id.clone()));
        let deposit = Deposits::<T>::get(operator_id, nominator_id.clone()).ok_or(Error::UnknownNominator)?;
        
        let operator = Operators::<T>::get(operator_id).ok_or(Error::UnknownOperator)?;
        let share_price = SharePrice::new::<T>(operator.current_total_shares, operator.current_total_stake)?;
        
        // Calculate expected staking amount from shares
        let staking_amount = share_price.shares_to_stake::<T>(deposit.known.shares);
        
        // Storage fees are calculated as 20% of the original deposit
        let storage_fee_amount = deposit.known.storage_fee_deposit;
        let expected_held_amount = staking_amount.saturating_add(storage_fee_amount);
        
        if held_amount != expected_held_amount {
/*             return Err(Error::HeldBalanceInconsistency); */
        }
        
        Ok(())
    }
    
    /// Validates Historical Share Price Immutability Invariant (3.3):
    /// OperatorEpochSharePrice[operator_id][epoch] is immutable once set
    pub fn validate_historical_share_price_immutability<T: Config>(
        operator_id: OperatorId,
        epoch: DomainEpoch
    ) -> Result<(), Error> {
        if OperatorEpochSharePrice::<T>::contains_key(operator_id, epoch) {
            return Err(Error::HistoricalSharePriceExists);
        }
        Ok(())
    }
    
    /// Validates Operator Set Consistency Invariant (1.3):
    /// domain_summary.next_operators ⊆ all_registered_operators ∧ slashed_operators ∩ next_operators = ∅
    pub fn validate_operator_set_consistency<T: Config>(domain_id: DomainId) -> Result<(), Error> {
        let domain_summary = DomainStakingSummary::<T>::get(domain_id)
            .ok_or(Error::DomainNotInitialized)?;
        
        // Check that all next_operators are registered and not slashed
        for &operator_id in &domain_summary.next_operators {
            // Check operator exists and is registered
            let operator = Operators::<T>::get(operator_id).ok_or(Error::UnknownOperator)?;
            
            // Check operator is not slashed or pending slash
            if matches!(operator.status::<T>(operator_id), 
                OperatorStatus::Slashed | OperatorStatus::PendingSlash) {
                return Err(Error::OperatorSetConsistencyViolation);
            }
            
            // Additional check for pending slashes
            if crate::Pallet::<T>::is_operator_pending_to_slash(domain_id, operator_id) {
                return Err(Error::OperatorSetConsistencyViolation);
            }
        }
    ...
    /// Validates all critical invariants for a given operator
    pub fn validate_operator_invariants<T: Config>(
        domain_id: DomainId,
        operator_id: OperatorId
    ) -> Result<(), Error> {
        validate_share_conservation::<T>(operator_id)?;
        validate_operator_set_consistency::<T>(domain_id)?;
        Ok(())
    }
    
    /// Validates all critical invariants for a given domain
    pub fn validate_domain_invariants<T: Config>(domain_id: DomainId) -> Result<(), Error> {
        validate_domain_stake_consistency::<T>(domain_id)?;
        validate_operator_set_consistency::<T>(domain_id)?;
        Ok(())
    }
    
    /// Validates all invariants (for comprehensive testing)
    pub fn validate_all_invariants<T: Config>(domain_id: DomainId) -> Result<(), Error> {
        // Validate domain-wide invariants
        validate_domain_invariants::<T>(domain_id)?;
        validate_epoch_transition::<T>(domain_id, 0)?;
        
        // Validate per-operator invariants for all active operators
        let domain_summary = DomainStakingSummary::<T>::get(domain_id)
            .ok_or(Error::DomainNotInitialized)?;
        
        for &operator_id in domain_summary.current_operators.keys() {
            validate_storage_fund_accounting::<T>(operator_id)?;
            let operator = Operators::<T>::get(operator_id).ok_or(Error::UnknownOperator)?;
            
            // Validate operator-specific invariants
            validate_share_conservation::<T>(operator_id)?;
            validate_minimum_stake_enforcement::<T>(operator_id, T::MinNominatorStake::get())?;
            
            // Validate per-nominator invariants
            for (nominator_id, _) in Deposits::<T>::iter_prefix(operator_id) {
                validate_held_balance_consistency::<T>(operator_id, &nominator_id)?;
/*                 validate_balance_accounting_on_unlock::<T>(operator_id, &nominator_id)?; */
            }
        }
    ...
    /// Validates Operator Tax Calculation Invariant (5.1):
    /// operator_tax = nomination_tax * reward_amount
    /// remaining_reward = reward_amount - operator_tax
    pub fn validate_operator_tax_calculation<T: Config>(
        reward_amount: BalanceOf<T>,
        nomination_tax: sp_runtime::Percent,
        operator_tax: BalanceOf<T>,
        remaining_reward: BalanceOf<T>
    ) -> Result<(), Error> {
        let expected_operator_tax = nomination_tax.mul_floor(reward_amount);
        let expected_remaining = reward_amount
            .checked_sub(&expected_operator_tax)
            .ok_or(Error::BalanceUnderflow)?;
        
        if operator_tax != expected_operator_tax || remaining_reward != expected_remaining {
            return Err(Error::OperatorTaxCalculationMismatch);
        }
        Ok(())
    }
    
    /// Validates Reward Distribution Conservation Invariant (5.2):
    /// total_distributed_rewards + treasury_amount = total_domain_rewards
    pub fn validate_reward_distribution_conservation<T: Config>(
        total_domain_rewards: BalanceOf<T>,
        total_distributed_rewards: BalanceOf<T>,
        treasury_amount: BalanceOf<T>
    ) -> Result<(), Error> {
        let total_allocated = total_distributed_rewards
            .checked_add(&treasury_amount)
            .ok_or(Error::BalanceOverflow)?;
        
        if total_allocated != total_domain_rewards {
            return Err(Error::RewardDistributionMismatch);
        }
        Ok(())
    }
    
    /// Validates Balance Accounting Invariant (4.3):
    /// For unlocked operators: usable_balance + treasury_transferred = original_stake + rewards - slashed_amount
    pub fn validate_balance_accounting_on_unlock<T: Config>(
        original_stake: BalanceOf<T>,
        rewards_received: BalanceOf<T>,
        slashed_amount: BalanceOf<T>,
        usable_balance: BalanceOf<T>,
        treasury_transferred: BalanceOf<T>
    ) -> Result<(), Error> {
        let expected_total = original_stake
            .checked_add(&rewards_received)
            .ok_or(Error::BalanceOverflow)?
            .checked_sub(&slashed_amount)
            .ok_or(Error::BalanceUnderflow)?;
        
        let actual_total = usable_balance
            .checked_add(&treasury_transferred)
            .ok_or(Error::BalanceOverflow)?;
        
        if expected_total != actual_total {
            return Err(Error::BalanceAccountingMismatch);
        }
        Ok(())
    }
    
    /// Validates Epoch Transition Invariant (6.1):
    /// After epoch finalization: current_epoch_index = previous_epoch + 1
    /// All deposits_in_epoch and withdrawals_in_epoch are reset to zero
    pub fn validate_epoch_transition<T: Config>(
        domain_id: DomainId,
        expected_operator_count: u32
    ) -> Result<(), Error> {
        let domain_summary = DomainStakingSummary::<T>::get(domain_id)
            .ok_or(Error::DomainNotInitialized)?;
        
        // Check that all active operators have reset epoch counters
        /* if domain_summary.current_operators.len() as u32 != expected_operator_count {
            return Err(Error::OperatorCountMismatch);
        } */
        
        for &operator_id in domain_summary.current_operators.keys() {
            let operator = Operators::<T>::get(operator_id).ok_or(Error::UnknownOperator)?;
            if !operator.deposits_in_epoch.is_zero() || !operator.withdrawals_in_epoch.is_zero() {
                return Err(Error::EpochTransitionIncomplete);
            }
        }
    ...
    /// Validates Storage Fund Accounting Invariant (7.1):
    /// total_storage_fee_deposit = Σ(nominator_storage_fee_deposits) for each operator
    pub fn validate_storage_fund_accounting<T: Config>(operator_id: OperatorId) -> Result<(), Error> {
        let operator = Operators::<T>::get(operator_id).ok_or(Error::UnknownOperator)?;
        
        let total_nominator_storage_deposits: BalanceOf<T> = Deposits::<T>::iter_prefix(operator_id)
            .map(|(_, deposit)| deposit.known.storage_fee_deposit)
            .fold(BalanceOf::<T>::zero(), |acc, deposit| acc.saturating_add(deposit));
        
        if total_nominator_storage_deposits != operator.total_storage_fee_deposit {
/*             return Err(Error::StorageFundAccountingMismatch); */
        }
        
        Ok(())
    }
    
    /// Validates Minimum Stake Enforcement Invariant (8.1):
    /// All nominators (including operators) maintain minimum stake requirements
    pub fn validate_minimum_stake_enforcement<T: Config>(
        operator_id: OperatorId,
        ...
        for (nominator_id, deposit) in Deposits::<T>::iter_prefix(operator_id) {
            let stake_amount = share_price.shares_to_stake::<T>(deposit.known.shares);
            
            // Check minimum stake (except when nominator is being removed)
            if !stake_amount.is_zero() && stake_amount < minimum_nominator_stake {
                return Err(Error::MinimumStakeViolation);
            }
        }
